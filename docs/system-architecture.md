...
| MG90S servos (metal gear) | PWM | Rotates dispenser; agitates chute |
...
4. **Timeout-driven escalation** — a missed dose is not silently dropped: it is logged with a `MISSED` event for the caregiver/study record. (Automatic re-alert is planned; see [firmware-guide.md §8](firmware-guide.md).)
